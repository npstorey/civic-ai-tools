...
The internal Typed Standards proposal (`typed-standards-specification.md`) §3 introduces a layered architecture distinguishing **Content Profiles** (typed-content carriers — Typed Claims / Typed Evidence / Typed Questions) from **Producer Profiles** (e.g., AI-Assisted Analysis). §9 explicitly flags an unresolved boundary:
...
- **captureMethod vocabulary.** The bundle declares which `metadata.captureMethod` values (per ADR-0003) are valid for packages conforming to this subtype's parent Producer Profile. The core spec keeps the field but no longer enumerates the values; the vocabulary is per-profile. Per [ADR-0011](0011-capturemethod-generalization.md), the v0.1 vocabulary for the `ai-assisted-analysis` Producer Profile is `chat-flow-stream` / `claude-code-jsonl-readback` / `claude-code-self-report` (the three values originally enumerated in OES §9 core by ADR-0003), declared at the profile-type level and inherited by all `ai-assisted-analysis` subtypes — including `ai-assisted-analysis/datHere` — unless a subtype's bundle explicitly constrains or extends the parent vocabulary. Future AI-publishing surfaces extend this vocabulary by amending the bundle; non-AI Producer Profiles (Human, Hybrid, Sandbox-only, future adopter profiles) declare their own captureMethod vocabularies in their respective guidance bundles when promoted from reserved to built.
