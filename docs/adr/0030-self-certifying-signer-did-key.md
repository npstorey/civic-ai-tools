...
3. On a match, when a supplied registry **does** list the envelope's `(kid, publicKey)`: the registry's verdict about that key stands — `active`, `deprecated_valid`, `deprecated_invalid` or `revoked` — and is reported as the key-trust status, because a publisher's statement about a key it lists outranks the key's statement about itself; `self_certified` is not reported. Check #14 then also applies its existing rule: a registry `signerIdentity.identifier` that differs from the claimed identifier is `signer_identity_mismatch`, fatal, and one that equals it is `ok`. A registry MAY list a self-certifying key precisely to say it is revoked.

Rule 3 is the only way a registry can raise a self-certified signer's status, and it raises it to what the registry says, no further.
...
| **A supplied registry** | A registry that lists the key read as vouching for an identity | §4 rule 3: only a registry entry matching `(kid, publicKey)` changes the status, and only to what the registry says. A registry that does not list the key changes nothing, and `unknown_key` is not reported for it (it would suggest the signer *should* have been listed). |
...
- **Reporting `self_certified` even when a registry lists the key.** Rejected: a publisher's revocation of a key it lists must be visible, and its `active` is a stronger statement than the key's own.
...
- **Verifier and core work.** P4: the encoder, the derivation, the predicate, the eighth status, the two check-#14 statuses, the canary pair. P5: the helper, the conditional in `buildCommitmentView` with its derivation check, one byte-golden case, a produce-then-verify round trip yielding `self_certified`. P6: `KEY_TRUST_SIGNALS` and `SIGNER_IDENTITY_SIGNALS` rows per §10 (`apps/web/src/lib/trust-signal.ts:262-308`, `:614-638`), which the type check forces. P7: the fixture package. P8: the cold read against §10.
