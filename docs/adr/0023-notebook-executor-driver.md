...
- **The environment inventory grows the executor's variables** — the driver selector, the container image tag, and (previously undocumented) the default driver's snapshot id and off-platform auth set — closing an env-inventory gap the sprint brief flagged.
