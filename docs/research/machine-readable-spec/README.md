...
This bundle frames the [Typed Standards Specification](../../architecture/typed-standards-specification.md) twice, in two established formalisms, to see what each one makes legible.
...
- **`ontology/typedClaims.ttl`** — a draft OWL ontology for the typed-claims layer (spec §8.11), research-draft and non-normative. Recorded in the open-questions registry ([Q10](../../architecture/open-questions.md#q10--civic-claim-vocabulary-as-a-full-ontology)) as the candidate artifact for the ontology question; adoption is gated on a further review round per that entry, and nothing here promotes it.
...
Exploratory research framing, produced 2026-06-24; **reconciled 2026-08-18** against the specification's 2026-08-03 revision (the v0.1.4 reconciliation, Appendix G) and the ADR corpus through [ADR-0024](../../adr/0024-evidence-path-configuration.md). The reconciliation carries forward, among other changes: the `sealed` / `public` visibility vocabulary and the three orthogonal disposition dimensions ([ADR-0016](../../adr/0016-vcs-native-lifecycle-mapping.md) §A, spec §8.10.6), the signing-status axis and unsigned dev tier ([ADR-0020](../../adr/0020-instance-key-custody.md)), the `attestation/revises/v1` sub-type and `vcsRef` field (spec §8.10.5, §8.1.1), the ratified §8.8.1 commitment-view shape, and the `@typedstandards/produce-core` / `@typedstandards/civic-typed-harness` package extractions ([ADR-0021](../../adr/0021-produce-core-extraction.md), [ADR-0022](../../adr/0022-civic-typed-harness-packaging.md)). The open-questions registry now records formalization-review positions touching this bundle (Q10, Q41); those remain positions, not resolutions, and nothing here treats them otherwise.

Two announced upstream changes are deliberately **not yet incorporated**: (1) a coordinated vocabulary rename will remove the word "evidence" from the protocol's public surface — wire field names, route segments, environment variables — landing as a spec patch revision with a full old→new mapping table and a legacy-alias window (names only, nothing structural; registry context: [Q50](../../architecture/open-questions.md#q50--evidence-product-framing-vs-precise-typed-node-resource-naming)). The bundle keeps the current names (`EvidencePackage`, `evidenceProtocolVersion`, the `urn:civic-evidence:` scheme, …) until that revision lands, then reconciles against its mapping table. (2) The specification's canonical markdown is expected to relocate from this repository to the typedstandards repository; the timing is being coordinated with this bundle so its spec pointers move with the file rather than chasing it.

This is **not** a normative addition to the Typed Standards Specification. It does **not** change the spec or any ADR, and it introduces no new requirements. It is a study artifact in the sense of the [xanadu-doctrine](../../architecture/xanadu-doctrine.md): the two framings are non-committal viewpoints, each with an explicit promotion trigger named in `03-formalism-comparison.md`, and neither has been promoted. The spec at [`docs/architecture/typed-standards-specification.md`](../../architecture/typed-standards-specification.md) remains the single source of truth; where this bundle and the spec disagree, the spec governs.
