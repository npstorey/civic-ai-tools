...
| **Two-family taxonomy** (`content/*` and `attestation/*`) | **partial** (yellow); per ADR-0009 | The semantic distinction over the primitive. `content/*` = standalone assertions (no `targetNodeId`). `attestation/*` = assertions about another node (`targetNodeId` required). `content/analysis/v1` = built; QEC typed sub-types (`content/claim/question/evidence/v1`) reserved; host/hostPolicy/hostTermsOfUse/tool sub-types reserved per Q22; the 16-row `attestation/*` sub-type table (§8.12.1 — the Q36-ratified 15 plus `attestation/revises/v1`, minted 2026-08-03 per ADR-0016 §C) is **ratified** but per-sub-type operationalization lands via downstream ADRs (treat individual attestation sub-types as **reserved/ratified**). Within `content/*`, the **QEC sub-ontology** (`metadata.contentType` set-valued over `claim`/`question`/`evidence`/`untyped`) is the most-developed sub-area. |
...
└── AttestationNode  (attestation/* — assertion about another node; targetNodeId REQUIRED ≥1)
    ...
    ├── attestation/supersedes/v1      [ratified] publisher-only (corrective replacement;
    │                                              deprecation signal — §8.10.5)
    ├── attestation/revises/v1         [ratified] publisher-only (neutral version succession;
    │                                              no deprecation signal; ADR-0016 §C, §8.10.5)
    ├── attestation/publishes/v1       [ratified] publisher-only OR delegated-publisher (Q20)
    │                                              (sealed → public visibility transition)
...
| `contentProfile` | string enum | 0..1 | no | `default` (absence ⇒ default) \| `datHere`. Orthogonal to captureMethod. Consistency invariant: `metadata.contentProfile === 'datHere'` iff `producerProfile.startsWith('ai-assisted-analysis/datHere')`. **Not a top-level field — lives at `metadata.contentProfile`** (§3.5). *(Placement ratified 2026-08-03: the spec's §8.1.2 placement note aligned the spec text to the shipped wire — `metadata.contentProfile` alongside `signingKeyId`/`captureMethod`, with `producerProfile` top-level as the successor axis; this model's earlier placement-ambiguity flag is resolved with zero wire change.)* |
...
| `vcsRef` | object `{repoUrl, commitSha, path?, ref?}` | 0..1 | no | Optional attested version-control self-declaration (§8.1.1; ADR-0016 §B): `repoUrl` (required-if-present), `commitSha` (required-if-present; full immutable revision object id), `path` (optional), `ref` (optional; mutable pointer, informative only). Top-level envelope field alongside `producerProfile`/`type`/`signer`, covered by envelope hash + signature — the signature attests the *assertion*, not the *fact*. Verification is verify-on-fetch: a verifier MAY resolve `repoUrl`+`commitSha` and check the artifact at `path` against the node's `contentHash`; mismatch/unreachable is **informative, not a hard failure** (mirrors `locatedAt`, §8.10.2); consumer weight on an unverified `vcsRef` is `captureMethod`-contextualized. Distinct from `locatedAt` (where the published artifact is fetchable vs. which source revision the analysis derives from). |
...
#### `SelfContainedCommitmentBundle` (`?inline=1`) — §8.8 / §8.8.1 / §9.4

Per the §8.8.1 serialization note (ratified 2026-08-03), the `?inline=1` bundle is the §8.8.1 commitment view itself plus two inlined fields (`package`, `trustRegistry`); the proof-side fields (`rfc3161Timestamp`, `rekorEntryBody` + `rekorInclusionProof`, `lifecycleAttestations`) are already carried inline by the default view, so the inline form verifies with zero network access.

| field | datatype | cardinality | required | constraints |
|---|---|---|---|---|
| `package` | object (canonical-JSON package) | 1 | yes | Conformant package (§8.1); otherwise fetched from `packageUrl`. |
| `trustRegistry` (stamped/inlined) | object | 1 | yes | The publisher's trust-registry document with its `generatedAt` as-of date (`.well-known` snapshot); check #5 verifies against this with no live fetch. |
| `rfc3161Timestamp` | RFC 3161 token | 0..1 | no | Inline in the default view when present; check #7. |
| `rekorEntryBody` + `rekorInclusionProof` | string (base64) + string (JSON) | 0..1 | no | Canonical Rekor leaf body + Merkle inclusion proof; check #8 verifies offline (§9.4). Legacy/calm packages may have no Rekor entry. |
| `lifecycleAttestations` | array of attestation/* envelopes | 0..* | no | Inlined signed chain (embed form); check #10; each signature verified in-process. |
| `generatedAt` (on `trustRegistry`) | dateTime | 1 | yes | As-of moment of the stamped registry; bounds revocation visibility. |

#### `CommitmentView` (cross-host publication logical schema) — §8.8.1

Two byte-different but semantically identical serializations: **NotebookEmbeddedCommitmentView** (§8.8.2; carried at notebook root `metadata.org.civicaitools.evidence`, MUST survive tooling round-trip) and **SiblingYamlCommitmentView** (§8.8.3; `<artifact-basename>.evidence.yaml`, or markdown YAML frontmatter; verifier MUST accept YAML or JSON). A conformant publisher MAY emit either; a conformant verifier MUST accept either. The required/conditional/optional marks below reflect the served reference shape ratified into §8.8.1 on 2026-08-03 (codebase-wins, zero wire change); absent optional fields are omitted, never emitted as `null` (the two explicitly-nullable exceptions are marked).
...
| `packageUrl` | string (URL) | 0..1 | conditional | Content-addressable URL of canonical-JSON package. Omitted when unknown and on redacted (sealed-visibility) views — a non-derivable capability URL MUST NOT be disclosed for a sealed-visibility record. |
| `visibility` | string enum | 1 | yes | `sealed` \| `public` (ADR-0016 §A vocabulary). Lets a verifier render a sealed / not-publicly-located state honestly instead of treating a missing `packageUrl` as an error. Legacy values `committed`/`published` are accepted **input aliases** (`committed` → `sealed`, `published` → `public`); reference implementations predating the value-rename sweep emit the legacy values, and consumers SHOULD accept them (§8.8.1). |
| `captureMethod` | string (enum, open) \| null | 1 | yes | Mirrors `metadata.captureMethod`. Open-ended ("at minimum" the ai-assisted-analysis v0.1 vocab). Emitted as an explicit `null` when the record predates the capture-method discipline. |
| `contentProfile` | string | 1 | yes | Mirrors `metadata.contentProfile` (§8.1.2); `"default"` when the package carries none. |
| `producerProfile` | string | 0..1 | no | Mirrors the package's top-level `producerProfile` (§8.1.1). Absent on packages predating it. |
| `type` | string (URI) | 0..1 | no | Mirrors the package's `type`. Absence ⇒ `content/analysis/v1`. |
| `signer` | Signer object (§8.5 shape) | 0..1 | no | The envelope-side identity claim `{bindingTier, identifier, displayName, verifiedAt?}`, mirrored verbatim from the package — **the subject of §9.2 check #14**. Distinct from `signerIdentity` below (see the naming note). Absent on pre-v0.1 packages. |
| `contentHash` | ContentHash multihash object | 0..1 | no | Mirrored from the canonical JSON (§8.2). Absent on pre-v0.1 packages. |
| `contentCanonicalization` | string (URI) | 0..1 | no | The package's content-canonicalization rule URI. Absent on pre-v0.1 packages. |
| `signature` | SignedEnvelope object | 0..1 | conditional | Carried **verbatim**: `{signature, publicKey, algorithm, kid}` per §8.3.1. `algorithm` is load-bearing — a verifier MUST dispatch Ed25519 vs. Ed25519ph on it; `algorithm`/`kid` MAY be absent on packages signed via older paths. Omitted when the package is unsigned (§8.3.1 best-effort signing). |
| `signerIdentity` | object | 0..1 | no | **Informational** provider-identity block about the record's creator (reference impl: publishing user's public GitHub identity `{provider, providerId, displayName, profileUrl}`). Human context only — **MUST NOT be used as the signature subject** (that is `signer` above). |
...
| `rekorInclusionProof` | string (JSON) | 0..1 | no | Present when pipeline obtains one. |
| `rekorEntryBody` | string (base64) | 0..1 | no | Canonical Rekor leaf body, carried so Merkle inclusion verifies offline (§9.4) without fetching the log entry. |
| `lifecycle` | object | 0..1 | no | Informational summary `{status: "active" \| "withdrawn", withdrawnAt?, withdrawnReason?, reinstatedAt?, reinstatedReason?}`. Omitted when no lifecycle history. Authoritative state is the signed attestation chain (§8.10). |
| `lifecycleAttestations` | array of attestation/* envelopes | 0..* | no | Signed lifecycle envelopes inline (embed form) so the §9.2 check-#10 chain resolves offline. Omitted when empty. |
| `attestations` | array of AttestationEntry | 0..* | no | **Non-lifecycle** entries only (corroborations, contradictions, evaluations, …); each reference-form OR embed-form (§8.9). The lifecycle chain travels under `lifecycleAttestations`. |
| `trustRegistryUrl` | string (URL) | 1 | yes | Canonical `.well-known/typed-publisher.json` URL. Lets reader resolve `signature.kid` independently of publishing host. Per-publisher configuration — never a constant. |
| `trustRegistryUrlLegacy` | string (URL) | 0..1 | no | Secondary registry URL served byte-identical at the §8.3.3 legacy path (`.well-known/evidence-public-keys.json`), for clients that only know the older path. |
| `subjectTitle` | string \| null | 0..1 | conditional | Matches publisher DB title. Omitted on redacted (sealed-visibility) views; MAY be an explicit `null` when the publisher has no title. |
| `subjectSummary` | string \| null | 0..1 | conditional | G-section summary; matches canonical-JSON `summary`. Omitted on redacted (sealed-visibility) views. |

> **`signer` vs. `signerIdentity` (naming note, ratified 2026-08-03).** Three `signerIdentity` surfaces are disambiguated: (1) a **trust-registry entry's** `signerIdentity` (§8.3.3) — §8.5-shaped, resolved via the envelope's `kid`, one side of check #14; (2) the commitment view's **`signer`** — the §8.5-shaped claim mirrored from the package, the other side of check #14; (3) the commitment view's **`signerIdentity`** — informational, MUST NOT be used as the signature subject.
>
> **Redaction rule (sealed-visibility records).** The commitment itself is public by design (the envelope hash is already on the transparency log), but a sealed record's content surface is not: a redacted view omits `packageUrl`, `subjectTitle`, `subjectSummary`; the proof-side fields are served unredacted — they ARE the commitment.
>
> **`?inline=1` serialization.** This same view plus inlined `package` + `trustRegistry` = the self-contained bundle above; verifies with zero network access.
...
| `attestation/supersedes/v1` | publisher-only | `targetNodeId` (prior/old): nodeId, 1, yes · `successorNodeId` (new): nodeId, 1, yes. **Corrective replacement** — carries a deprecation signal (consumers should stop relying on the superseded node); contrast `revises` (§8.10.5, §8.12.1) |
| `attestation/revises/v1` | publisher-only (the lineage owner) | `targetNodeId` (prior revision — the parent): nodeId, 1, yes · `successorNodeId` (this revision — the child): nodeId, 1, yes. **Neutral version succession** — NO deprecation signal; the prior revision stays a valid point-in-time snapshot. Single-parent/linear at v0.1 (merge-DAG lineage deferred). The diff between two revisions is a **derivable human view, not a signed object** (§8.10.5; minted per ADR-0016 §C, 2026-08-03) |
| `attestation/publishes/v1` | publisher-only OR delegated-publisher (Q20) | `targetNodeId`: nodeId, 1, yes · `publicationHost`: string, 1, yes · `releasedAt`: timestamp, 1, yes. Transitions visibility `sealed` → `public` (§8.10.6) |
| `attestation/locatedAt/v1` | any-with-binding | `targetNodeId`: nodeId, 1, yes · `uri`: URI, 1, yes · `targetContentHash`: multihash, 1, yes (SHOULD match target's `contentHash`; mismatch is informative content-drift, NOT a failure; named `targetContentHash` because the structural primitive claims `contentHash` for the attestation node's own off-log fingerprint — Q48 resolved 2026-08-03, §8.10.2) · `contentLength`: integer, 0..1, no · `availability`: string/enum (unspecified), 0..1, no |
...
| `contentProfile` | string enum | 0..1 | no | `default` (absence ⇒ default) \| `datHere`. **This is the field's home** — retained as legacy alias v0.1 beside top-level `producerProfile`; verifiers SHOULD prefer `producerProfile`; the two MUST be consistent. Placement ratified 2026-08-03 (spec §8.1.2 placement note; this model already documented the wire correctly — see §3.1). |
...
37. **C-all-inpackage-signed** | EvidencePackage | crypto | All spec-defined field values (captureMethod, signingKeyId, contentProfile, producerProfile, contentCanonicalization, contentHash, vcsRef, every extensions entry, BlobRef objects) are part of the canonical JSON, JCS bytes, envelope hash, and signature. | **MUST** |
...
95. **C-SUPERSEDES-FIELDS / C-REVISES-FIELDS / C-PUBLISHES-FIELDS** | Supersedes/Revises/Publishes | structural | supersedes carries `targetNodeId`(old)+`successorNodeId`(new) — corrective replacement, deprecation signal; revises carries `targetNodeId`(prior revision)+`successorNodeId`(this revision) — neutral version succession, NO deprecation signal, prior revision stays a valid point-in-time snapshot, single-parent/linear at v0.1 (§8.10.5; ADR-0016 §C); publishes carries `targetNodeId`+`publicationHost`+`releasedAt` (sealed → public). | **MUST**
...
97. **C-LOCATEDAT-FIELDS / C-LOCATEDAT-TARGETCONTENTHASH** (formerly -CONTENTHASH) | AttestationLocatedAtV1 | structural/value | Carries `targetNodeId`, `uri`, `targetContentHash` (multihash), optional `contentLength`/`availability`; `targetContentHash` SHOULD match the target's `contentHash` — mismatch is informative content-drift, NOT a failure. Payload-field name ratified as `targetContentHash` 2026-08-03 (Q48 resolved): the structural primitive claims `contentHash` for the attestation node's own fingerprint, so the two meanings cannot share one key (same disambiguation pattern as `targetNodeId`). | **MUST** / **SHOULD**
...
101. **C-COMMIT-REQUIRED-FIELDS / C-COMMIT-REDACTION-SEALED / C-COMMIT-SIGNER-SUBJECT** | CommitmentView | cardinality/conditional/value | Per the §8.8.1 field set ratified 2026-08-03 — Required (1..1): evidenceProtocolVersion, packageHash, visibility (`sealed`\|`public`; legacy input aliases `committed`→`sealed`, `published`→`public` accepted — pre-rename-sweep implementations emit the legacy values), captureMethod (string\|null), contentProfile, trustRegistryUrl. Conditional: packageUrl (omitted when unknown and on redacted sealed-visibility views), signature (omitted when unsigned), subjectTitle/subjectSummary (nullable; omitted on redacted views). Optional: producerProfile, type, signer, contentHash, contentCanonicalization, signerIdentity, rfc3161Timestamp, rekorEntryId, rekorInclusionProof, rekorEntryBody, lifecycle, lifecycleAttestations, attestations (non-lifecycle entries only), trustRegistryUrlLegacy. Redaction rule: a sealed-visibility redacted view omits packageUrl/subjectTitle/subjectSummary; proof-side fields served unredacted. Identity subject: `signer` is the §9.2 check-#14 subject; `signerIdentity` is informational and MUST NOT be used as the signature subject. | **MUST** / **MUST NOT**
...
112. **C-MULTICYCLE-NO-LIMIT** | LifecycleAttestation | cardinality | `public → withdrawn → reinstated → withdrawn → …` is a longer chain; each reinstatement points back to its immediately-prior withdrawal via `priorWithdrawalNodeId`; no cycle counter, no DB-shape mutation per cycle, no spec-level cycle limit. | **MAY**
...
114. **C-RETENTION-WITHDRAWAL-BOUNDED** | WithdrawsAttestation | structural | A withdrawal from publisher P retracts P's own standing (lifecycle-status label) only — not the content's disclosure (visibility stays `public` for a formerly-public node, §8.10.6); it does NOT invalidate `locatedAt` attestations signed by other parties pointing at the content elsewhere. | **MUST**
...
149. **C-VCSREF-ATTESTED-ASSERTION / C-VCSREF-VERIFY-ON-FETCH** | EvidencePackage.vcsRef | crypto/conditional | `vcsRef` (§8.1.1; ADR-0016 §B) is an attested self-declaration covered by envelope hash + signature — the signature attests the *assertion*, not the *fact* (it proves the signer asserted the analysis corresponds to this revision, not that the revision exists or matches). Verify-on-fetch: a verifier MAY resolve `repoUrl`+`commitSha` and compare the artifact at `path` against the node's `contentHash`; a mismatch or unreachable revision is informative, NOT a hard failure (mirrors locatedAt, §8.10.2); consumer weight on an unverified `vcsRef` is `captureMethod`-contextualized. Mitigates the §10.1 **false-VCS-binding publisher** adversary (pre-signing-fabrication class), alongside the kid-swap attacker of #143. | **MAY** / **SHOULD**
...
**Orthogonal dimensions (§8.10.6; ADR-0016 §A.1 + ADR-0020 §B).** A content node's disposition decomposes into independent axes — modeled here as separate state variables, never one composite state machine:

| Dimension | Question | Values | Derived from |
|---|---|---|---|
| **Signing status** (ADR-0020 §B) | Is there an attributable commitment? | `unsigned` → `signed` | key configuration; Ed25519ph signing. Orthogonal to visibility — NOT a third visibility value. |
| **Visibility** | Is the content disclosed? | `sealed` / `public` | presence of `attestation/publishes/v1` + `attestation/locatedAt/v1` (zero-`locatedAt` = the sealed base case, §8.10.2) |
...
| **Host display** | Does *this* host show it? | host's choice | host policy (`content/hostPolicy/v1`, Q22) reading the signals above — a host property, not a node property |

Value-set discipline: the visibility values are `sealed`/`public` per ADR-0016 §A (rename from the legacy `committed`/`published`; the legacy values are accepted **input aliases** indefinitely — pre-rename-sweep reference implementations emit them (§8.8.1); "publish"/"seal" remain the *verbs*, `attestation/publishes/v1` the relationship, and the cryptographic "commitment" noun of §8.8 is unaffected). `withdrawn`/`reinstated`/`superseded` are lifecycle status, NOT visibility values: a withdrawn-formerly-public node is **`public` + `withdrawn`** — withdrawal retracts the publisher's *standing*, not the content's *disclosure* (§8.10.3), and a publisher cannot un-disclose content already on the public log. On the signing axis, `sealed` is *defined* as signed + RFC 3161-timestamped + Rekor-logged (Rekor logging is signature-gated), so an **unsigned package can reach neither `sealed` nor `public`** — it is the pre-commitment condition, confined to local produce-and-inspect (ADR-0020 §B-C). *Availability* (whether a `locatedAt` URI currently resolves) rides the optional `availability` sub-field on `attestation/locatedAt/v1`, distinct again from visibility, which records asserted disclosure rather than current reachability.

### 5.1 Content-node visibility transition & lifecycle status (publication / withdrawal / reinstatement / supersession / revision)

| Entity | from → to | trigger | guard |
|---|---|---|---|
| ContentNode.visibility | sealed → public | `attestation/publishes/v1` (signed) | publisher-only OR delegated-publisher (Q20); carries `publicationHost` + `releasedAt`; references target by nodeId. Legacy state labels `committed`/`published` are accepted input aliases only |
| ContentNode.lifecycleStatus | active → withdrawn | `attestation/withdraws/v1` (signed) | publisher-only (`signer.identifier` == target's); `reason` required non-empty; `effectiveAt` defaults to envelope timestamp. Visibility unchanged — a withdrawn-formerly-public node stays `public` (§8.10.6) |
| ContentNode.lifecycleStatus | withdrawn → active (reinstated) | `attestation/reinstates/v1` (signed) | publisher-only; references target by nodeId AND prior withdrawal by `priorWithdrawalNodeId`; optional reason |
| ContentNode.lifecycleStatus | active (reinstated) → withdrawn (subsequent cycle) | `attestation/withdraws/v1` (signed) | publisher-only; multi-cycle by construction — no cycle counter, no DB-shape mutation, no spec-level limit |
| ContentNode.lifecycleStatus | active → superseded | `attestation/supersedes/v1` (signed) | publisher-only (typically same publisher); references prior by `targetNodeId` AND successor by `successorNodeId`; **corrective replacement** — deprecation signal; not stated mutually exclusive with withdraw/reinstate |
| RevisionLineage (no lifecycle-status change) | prior revision → successor revision | `attestation/revises/v1` (signed; ADR-0016 §C) | publisher-only (the lineage owner); references prior by `targetNodeId` AND successor by `successorNodeId`; **neutral version succession** — NO deprecation signal, the prior revision stays a valid point-in-time snapshot (neither withdrawn nor superseded merely because a successor exists); single-parent/linear at v0.1; the revision diff is a derivable human view, not a signed object (§8.10.5) |
...
| Entity | from → to | trigger | guard |
|---|---|---|---|
| ContentNode.visibility | sealed (private/draft/enterprise base case; zero locatedAt, §8.10.2) → public (located) | `attestation/locatedAt/v1` (signed) | any-with-binding; carries `targetNodeId`, `uri`, `targetContentHash` (SHOULD match target's `contentHash`; Q48-resolved name), optional `contentLength`/`availability` |
| ContentNode.visibility | public (located) → public (additional independent durable copy) | additional `locatedAt` from a different `(signer.identifier, uri-authority)` pair | distinct pairs signal independent durable copies (Q38); no `copyOf` minted |
| ContentNode.retention (asymmetry §8.10.3) | withdrawn-by-publisher-P → withdrawn-by-P but copy-verifiable-at-backup-B | P's withdraws does NOT retract backup-host B's locatedAt | B's signature still verifies; content at B's URI still hashes to the target's `contentHash`; B's locatedAt not retracted (only P's lifecycle status changed — the node stays `public`). Verifier MUST surface both. |
...
| content/claim/v1 node | (none) → sealed + active | publisher signs + emits the envelope (Ed25519ph, timestamp, Rekor, identity binding) | valid JSON-LD 1.1 claim payload conforming to the five §8.11.3 conditions; `public` follows via `publishes`/`locatedAt` per §5.1/§5.2 |
| content/claim/v1 node | active → withdrawn / withdrawn → active / active → superseded (lifecycle status; visibility unchanged) | §8.10 attestations targeting the claim by nodeId | inherits §8.10 authorization rules; claim verifiable/withdrawable/reinstatable/supersedable/corroboratable/contradictable independently of the source analysis |

### 5.4 Signing status & proof state (publish-time, best-effort; signing axis per ADR-0020 §B)

| Entity | from → to | trigger | guard |
|---|---|---|---|
| PackageSigningStatus (the ADR-0020 axis, orthogonal to visibility) | unsigned (dev / pre-commitment) → signed | signing key configured; Ed25519ph signing succeeds | satisfies signed-package conformance. An **unsigned** package carries no signature and — Rekor logging being signature-gated — no transparency-log entry, hence no attributable commitment: it can reach **neither `sealed` nor `public`** (`sealed` is defined as signed + RFC 3161-timestamped + Rekor-logged) and is confined to local produce-and-inspect. Intentional first-run dev tier; verifiers render a calm no-signing-key signal, never "commitment verified" |
| PackageSigningState | unsigned-envelope → null-signature (persisted, unsigned) | signing leg fails (best-effort) | package + envelope hash remain valid but NOT signed-package conformant; same neither-sealed-nor-public consequence as the intentional unsigned tier |
| TimestampState | signed → timestamped+logged | RFC 3161 token + Rekor proof obtained (best-effort) | on failure persists as null columns; package remains queryable. (A bare RFC 3161 existence-timestamp is not signature-gated — it proves "this hash existed at time T," not attribution) |
...
**Supplementary (not in the §9.2 numbered sequence): `vcsRef` verify-on-fetch (§8.1.1).** When the envelope carries the optional `vcsRef`, a verifier MAY resolve `repoUrl` + `commitSha` and check the artifact at `path` against the node's `contentHash`. Mismatch or unreachable revision is **informative, not a hard failure** (surfaced as a signal per §5.1, mirroring `locatedAt`'s content-drift treatment); the weight a consumer places on an *unverified* `vcsRef` is `captureMethod`-contextualized. Out-of-band (network fetch + hash recomputation) — not SHACL-expressible beyond field-shape presence.
...
| CommitmentView shape & values | all §8.8.1 fields with datatypes/cardinalities and conditional marks (packageUrl/signature/subjectTitle/subjectSummary conditional; sealed-visibility redaction rule); `visibility` ∈ {sealed, public} (legacy input aliases committed/published accepted; pre-rename-sweep implementations emit them); `signature.algorithm` dispatched (Ed25519 vs Ed25519ph); `evidenceProtocolVersion=='0.1.0'`; captureMethod ∈ known vocab or explicit null (pre-discipline); `attestations` = non-lifecycle entries only (lifecycle chain under `lifecycleAttestations`) | missing/malformed field; value out of permitted set; `signerIdentity` used as signature subject (MUST be `signer`) |
...
| R-vcsRef-attested / R-vcsRef-verify-on-fetch | `vcsRef` (optional, §8.1.1) is an attested self-declaration covered by envelope hash + signature (the signature attests the assertion, not the fact); a verifier MAY resolve `repoUrl`+`commitSha` and compare against `contentHash` — mismatch/unreachable is informative, NOT a hard failure; consumer weight on an unverified `vcsRef` is `captureMethod`-contextualized. | package / verifier / consumer | MAY / SHOULD |
...
| R-8.10-lifecycle-by-nodeId / R-8.10.1-* | Lifecycle and location attestations are separately-signed `attestation/*` nodes referencing the target by nodeId; withdraws/reinstates/supersedes/revises are publisher-only (withdraws: reason required non-empty + effectiveAt default; reinstates: priorWithdrawalNodeId + optional reason; supersedes: targetNodeId + successorNodeId — corrective replacement; revises: targetNodeId (prior revision) + successorNodeId — neutral succession, no deprecation signal, single-parent/linear v0.1 per §8.10.5); publishes is publisher-only OR delegated-publisher (Q20) with publicationHost + releasedAt (sealed → public); each lifecycle envelope is Ed25519ph-signed and SHOULD be timestamped + Rekor-included. | lifecycle/location attestations | MUST / SHOULD |
...
| R-8.10.2-locatedAt-auth / R-8.10.2-blobref-crosshost | `attestation/locatedAt/v1` references the node by nodeId, carries targetNodeId/uri/targetContentHash (SHOULD match the target's contentHash; Q48-resolved name)/optional contentLength/availability, authorization any-with-binding; cross-host declarations by non-parent-signers SHOULD use locatedAt instead of BlobRef. | locatedAt / publishers | MUST / SHOULD |
...
| **Q16** | Formal conformance criteria | **Open.** Formal conformance criteria, a reference test corpus, and a conformance-claims registration mechanism remain deferred. Sharpened by a 2026-07-22 cross-runtime feasibility review: for an independent implementation the highest-risk conformance surface is **RFC 8785 JCS parity** (number formatting, UTF-16 key ordering, escaping), not the signature layer; test vectors should pin canonical *bytes* for the awkward cases. |
...
| **Q36** | Attestation sub-type collapse | The **Q36-ratified** `attestation/*` table (15 sub-types at ratification; now 16 rows with `attestation/revises/v1` minted 2026-08-03 per ADR-0016 §C), with three refinements: `extractsTo` merges into `wasDerivedFrom` (AnalyticalDerivation = content-shape variant); `endorses` and `corroborates` stay distinct; Q38 resolves "locatedAt suffices, no copyOf." |
...
| **Q48** | `attestation/locatedAt/v1` payload fingerprint-field name collision | **Resolved (2026-08-03)** by the spec's v0.1.4 reconciliation: the payload field is ratified as **`targetContentHash`** (NOT `contentHash` — the structural primitive claims that key for the node's own off-log fingerprint), following the `targetNodeId` disambiguation precedent. The reference implementation already emitted the ratified name; no wire change. This model's §3.4 and §4.9 use the ratified name. |

**Also recorded as resolution bases (not open):** ADR-0008 (multihash `contentHash`), ADR-0009 (§7 ratified attestation table + sig-vs-signer cross-check), ADR-0005 (notebook `provenance` auto-emission), ADR-0003 (captureMethod vocabulary, relocated to the ai-assisted-analysis profile bundle), ADR-0006 (Producer Profile subtype/flavor model), ADR-0016 (visibility value rename `committed`→`sealed` / `published`→`public`; `vcsRef` envelope field; `attestation/revises/v1` + §8.10.5 lineage; §8.10.6 orthogonal dimensions), ADR-0020 (Q56 **instance case** resolved — per-instance key custody + the unsigned→signed signing-status axis; the spoke case remains open, re-scoped to a hub issue), ADR-0021 (Q59 **resolved** 2026-08-01 — `@typedstandards/produce-core` extraction; producer and verifier share one canonicalization/hash chain by construction).
