...
*Last updated: 2026-07-29 (v12) — purpose block expanded into a problem/solution intro: grounded mission recognition (org-profile phrasing), provenance-gap statement, solution framing; Typed Standards definition sentence and attribution line unchanged. v11 `b679e83`; v10 `356550c`; v9 `8acc9f3`; v8 `a8b9472`; v7 `fce0c0a`; v6 `7435fee`; v5.1 `deb9b5c`; v5 `49bcc8d`; v4 archived (`rulespec-interop-poc-v4-archive.md`, `3eeea0e`); v3 `f74ad55`; v2 `60b9345`; v1 `dbe1d2d`.*
...
Your organization states its aim plainly — "The world's rules, encoded": open, machine-readable encodings of statutes, regulations, and policy rules, built jurisdiction by jurisdiction and compiled into executable programs. It is an aim this project shares from the adjacent ground of machine-readable government information. This memo exists to make one observation about the stack that aim rests on: the provenance layer is present but underdeveloped. The deterministic artifacts are published, and in the NZ pilot they are witnessed with unusual honesty — the corpus journal, the gate tiers, the DID-NOT-RUN-with-reason discipline — which is a genuinely good start; but how each encoding came to be goes unrecorded, and trust in machine-readable law ultimately needs both: custody of the bytes, and attestation of the production process.

[Typed Standards](https://typedstandards.org) is a content-agnostic open standard for production-process attestation of analytical artifacts: a signed, content-addressed, capture-method-labeled record of how an artifact was produced, verifiable by a third party who does not trust the publisher ([specification](https://github.com/npstorey/civic-ai-tools/blob/main/docs/architecture/typed-standards-specification.md)). It fills exactly that gap — per artifact, without changing your formats, your keys, or your journals — and this memo demonstrates how. The [Civic AI Tools](https://civicaitools.org) project carries its reference implementation.
