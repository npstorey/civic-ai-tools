<!-- v3 — 2026-07-28 — adds the spec coverage map, the external-anchors boundary framing, diagram reading notes, and adjacent-standards/commit-later framing (all cited to the spec + summary). v2 at 60b9345; v1 at dbe1d2d. -->
...
*Last updated: 2026-07-28 (v3) — adds the spec coverage map, the external-anchors boundary framing, the diagram reading notes, and the adjacent-standards / commit-later framing, all cited to the spec and summary. v2 (the front-loaded restructure) at `60b9345`; v1 (comparison + verdicts) at `dbe1d2d`.*
...
*Reading notes: every edge converges on the seam — the one digest both systems publish independently, with no negotiation between them. The NEITHER lane is load-bearing: both verifiers disclaim those properties in their own output, quoted verbatim in [the two verdicts](#the-two-verdicts).*
...
| Verification flow (§9.2 ordered checks, offline) | **Exercised as the offline subset** | fetch stubbed to throw, zero network calls attempted; both verifiers' own scope disclaimers quoted in [the two verdicts](#the-two-verdicts) |
