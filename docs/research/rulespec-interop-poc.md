...
Your organization states its aim plainly — "The world's rules, encoded" — and for an aim like that, commitments to transparency, including publishing the methodology by which each encoding was created, are what establish trust in the encodings. One layer of the traceability in RuleSpec remains underdeveloped: the deterministic artifacts are published — in the NZ pilot, witnessed with unusual honesty — but how each encoding came to be goes unrecorded, and trust in machine-readable law needs both custody of the bytes and attestation of the production process.

One possible solution is [Typed Standards](https://typedstandards.org), a content-agnostic open standard for production-process attestation of analytical artifacts: a signed, content-addressed, capture-method-labeled record of how an artifact was produced, verifiable by a third party who does not trust the publisher ([specification](https://github.com/npstorey/civic-ai-tools/blob/main/docs/architecture/typed-standards-specification.md)).

*Prepared by Nathan Storey, creator of Typed Standards and of the [Civic AI Tools](https://civicaitools.org) project that carries its reference implementation, as an experimental proof of concept for Typed Standards filling that gap.*
...
The demonstration stands on this branch — three signed nodes, both verifiers passing offline, one negotiation-free digest join — and `./scripts/verify-rulespec-interop.sh` reproduces every claim from a clean checkout. What it establishes, what it deliberately does not, and the throwaway-key boundary that caps it are stated in [What a passing pair establishes — and what it does not](#what-a-passing-pair-establishes--and-what-it-does-not) and [Limitations](#limitations). From here, three concrete next steps:

1. **Exploring Typed Standards for new encoding events** — a node per encoding run, emitted at encode time in node 2's form — and possibly retroactive observation nodes for existing encodings, within the backfill bound stated in [What adopting would cost](#what-adopting-would-cost).
2. **Possible collaboration on a content-profile schema.** The speculative `axiom/statute-encoding/v0-sketch` on node 3 is a starting point, not a proposal: the envelope can already say what its content *is* — one of your statute encodings, produced under your declared disciplines — but nothing inspects that statement yet, and a real profile is a change we would make only through our formal decision process, and only with a committed adopter who needs it ([the worked example](#the-worked-example-executed--a-speculative-profile-on-a-real-node)).
3. **A frank discussion of whether this approach makes sense for RuleSpec** — whether production-process attestation in Typed Standards' form is a genuine value-add for your corpus, or the gap is better filled another way. The [findings index](#findings-index) — nine observations, nothing filed, awaiting triage on our side — is working material for that discussion.
...
*Speculative and non-normative — a research sketch, deliberately kept that way. Nothing below is registered; §9.2 check #15 would REJECT these `captureMethod` values today, by design; our own production publish API would 400 the `contentProfile` value below (`default` and `datHere` are the only accepted values). Promotion would be a deliberate, versioned change made only with a committed adopter — and the profile-bundle distribution mechanism it would ride on is itself still an open design question on our side.* Every element is discipline the corpus already practices, traceable to an artifact of yours the POC holds:
...
On `type`: `content/analysis/v1` sufficed for node 2, and a dedicated sub-type (e.g. a `content/encodingRun/v1`) is not obviously needed — the missing axis is the profile, not the type. Minting a sub-type would be a deliberate, versioned change on our side, made only if an adopter needs machine-discrimination between encoding runs and other analyses.
...
Observations only; nothing was filed. Full reasoning: v1 at `dbe1d2d` (finding 8 new in v4; finding 9 new in v9); each "to act" step waits on a real adopter's need, not on this POC.

| # | Finding | To act |
|---|---------|--------|
| 1 | No `captureMethod` value describes an encoder pipeline — §9.2 #15 *REJECTs* out-of-vocabulary values (`captureMethod_unknown`), so an honest label is structurally blocked, not merely absent; node 2 carries the nearest-fit chat label. Per the spec's axis separation this lands on the producer-profile axis — one gap with finding 2 | An amendment to a Producer Profile vocabulary bundle (its distribution mechanism is an open design question on our side); waits on a committed adopter |
| 2 | No Producer Profile for statute encoding — the other half of finding 1's gap ([sketch, now executed as node 3](#the-worked-example-executed--a-speculative-profile-on-a-real-node)) | Profile promotion per our profile-architecture decision record ([ADR-0006](../adr/0006-producer-profile-architecture.md)); waits on a committed adopter |
| 3 | No domain separation in §8.3.1 ([detail](#what-receipt-has-that-typed-standards-lacks)) | A deliberate, versioned spec change through our formal decision process — a domain tag changes signature bytes |
...
| 5 | §8.10 asserts append-only; §9.2 never verifies completeness ([detail](#what-receipt-has-that-typed-standards-lacks)) | A spec-level question for our formal decision process (it touches our open federation question) |
| 6 | The concepts in our lifecycle-mapping decision record ([ADR-0016](../adr/0016-vcs-native-lifecycle-mapping.md)) map cleanly onto your model (`revises`/`supersedes` ↔ journal corrections; `vcsRef` ↔ `subjectCommit`/toolchain pins; §8.10 lineage ↔ release chain) | Nothing — the abstractions were pitched right |
| 7 | Committed-mode friction, concrete: both nodes `visibility: committed` — commitment verifiable offline, bytes creator-distributed (by design in our visibility decision record, [ADR-0010](../adr/0010-visibility-lifecycle-location-attestations.md) §5; trades against the Rekor freshness lane, public in both visibility states) | Distribution-pattern question for a real committed-mode adopter |
| 8 | Seed material for our open design question on [formal conformance criteria](../architecture/open-questions.md#q16--formal-conformance-criteria): the two commitment fixtures with exact expected-verdict assertions, incl. the deliberately-not-green negative case | Observation only; held back until that question has a real consumer |
| 9 | `contentProfile` is signature-covered but semantically un-inspected — no §9.2 check reads, resolves, or warns about it, so node 3's unregistered sketch id is tamper-evident yet appears nowhere in the verdict ([the toggle](#the-two-verdicts-side-by-side); [worked example](#the-worked-example-executed--a-speculative-profile-on-a-real-node)) | Spec/verifier question — whether §9.2 should gain a contentProfile-resolution check (the #15 analog for the profile axis); held back until there is a registered profile a real adopter actually needs |
