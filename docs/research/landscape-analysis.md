...
| **Boston OpenContext** | [GitHub](https://github.com/CityOfBoston/OpenContext) | Generalized MCP-server framework (pure Python tool-server, Terraform-to-AWS deploy) from the City of Boston Department of Innovation and Technology. Production endpoint at [data-mcp.boston.gov/mcp](https://data-mcp.boston.gov/mcp). Ships 6 `ckan__`-prefixed tools against Analyze Boston (CKAN). No chat UI, no caching, no evidence — tool-server layer only. | Complementary, not competitive. Tool-server layer vs. civic-ai-tools' analysis + evidence layer. civicaitools.org consumes the Boston OpenContext endpoint in production as of v0.8.0. |
...
| **peoples-api-demos** (datHere) | [GitHub](https://github.com/dathere/peoples-api-demos) | Demo repo explicitly using MCP + Claude with [qsv](https://github.com/dathere/qsv) for civic-data chatbot flows (Pittsburgh parking, NYC snow, NYC housing). MCP-stack companion to the datHere AI chatbot product listed under Non-MCP Projects. | Peer — MCP-native implementation of the "People's API" chatbot pattern. Different data plane (qsv/CSV slicing) than civic-ai-tools' API-call plane (Socrata/Data Commons). Same architectural layer as civic-ai-tools skills. |
...
| **Democratic Capabilities Gap Map** (AI & Democracy Foundation) | [democracybuild.org](https://democracybuild.org) | Relational database mapping capabilities, research questions, and product gaps for deliberative democratic systems in the context of AI. Rates entries on five axes: maturity, importance, neglectedness, opportunity, transnational applicability. Maintained by the AI & Democracy Foundation ([ai-democracy.org](https://ai-democracy.org)); steering group Aviv Ovadya, Kyle Redman, Luke Thorhorn; funding from ARIA. Targets builders, researchers, funders, and policy practitioners. | Meta-directory / taxonomy, not a peer to position against. civic-ai-tools is a concrete instance that belongs *in* the gap map. civic-ai-tools#50 tracks submitting the project to the directory. |
| **Metagov `interop`** (Metagov) | [GitHub](https://github.com/metagov/interop) · [Metagov Theory & Practice](https://metagov.org/projects/theory-practice) | Metagov is a laboratory for internet-based self-governance. The `interop` initiative is an RFP-funded program (~$200K across 14 projects, July 2024 kickoff, final reports Jan 2025) that required grantees to implement standardized JSON / JSON-LD / CSV import-export for deliberative-governance tools. Supported by the Mina Foundation. | Structural analogue on an adjacent axis. Deliberative-tools-interop is not civic-data-evidence-interop, but the portable-format pattern parallels what a Path B spin-out of civic-ai-tools' evidence system would require. Reference point for future fork-resolution discussion, not a direct peer. |
...
### Policy / Vision Frameworks

| Initiative | Description | Relevance |
|-----------|-------------|-----------|
| **The Agentic State** (vision paper) | [agenticstate.org/paper.html](https://agenticstate.org/paper.html) | Published October 2025 by the Global Government Technology Centre Berlin and The World Bank. A 12-layer framework (6 implementation layers: public-service design, workflows, policy-making, regulatory compliance, crisis response, procurement; 6 enablement layers: governance, data and privacy, technical infrastructure, cybersecurity, public finance, organisational culture) for government adoption of agentic AI. Authors: Luukas Ilves, Manuel Kilian, Simone Maria Parazzoli, Tiago C. Peixoto, Ott Velsberg, with contributions from 20+ global digital-government leaders including Jennifer Pahlka (former US Deputy CTO) and Mykhailo Fedorov (Ukraine). | Different layer of the problem. The Agentic State is about *governments deploying agents* for service delivery, workflows, procurement, and so on. civic-ai-tools is about *legibility of AI outputs when citizens, journalists, or workers query open civic data*, regardless of the AI client. Enablement layers 7 (Agent Governance) and 8 (Data & Privacy) overlap most with civic-ai-tools' disclosure + evidence framing, but the stance is materially different. Complementary, not peer. Do not frame as "alignment." |
