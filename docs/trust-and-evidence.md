...
If you are comparing a record page against a downloaded bundle, expect the wording to differ. Both sets describe the same three values. Reconciling them is out of this document's scope — the strings live in the reference implementation and the neutral verifier, not in this repository.
...
| `metadata.createdAt`, `packageId`, `signingKeyId`, `contentHash`, `contentCanonicalization`, `type` | **Platform-derived at packaging time** — passed to or computed by the envelope builder (`packager.ts:432-459`; `contentHash` is derived inside `buildEnvelope`), identical under both methods | Same |
...
Three situations matter to a reviewer, and a government instance will plausibly meet all three: sealed drafts, published records, and an instance running before a signing key is configured. The first two are visibility states of a record; the third is a property of the instance, in which no record is produced at all.
...
**Sandbox-attested capture.** An opt-in path, intended for higher-stakes use, in which the analysis runs inside a controlled execution environment rather than a chat session. Tracked on the sibling website issue; not built.
...
**In the reference implementation, a review has carried a persisted signature since 2026-08-22** (migration 0016; fix `1171c2e`; every pre-existing row backfilled by `aafae07`). Before that date this section described a real gap — every review kind was hashed and stored, but the signing call ran and its result was discarded before reaching a column, so every review recorded from 2026-04-12 onward was unsigned while several product surfaces described the review as signed. That gap is now closed. Full account — the affected window, the fix, the backfill counts, and a public verification — is disclosed at [`disclosures.md`](disclosures.md#2-correctness-reviews-were-recorded-unsigned-while-four-surfaces-said-they-were-signed).

All three review kinds are still written by one route (`civic-ai-tools-website/src/app/api/evidence/[slug]/attestations/route.ts`), and still against `attestation_packages` rather than the ratified `attestation_nodes` table the specification models (§8.12.1, `:1305-1322`) — that migration remains separate, open work (Q73, below). What changed at the fix: the route now signs, timestamps, and persists before inserting the row, or refuses the write outright when a configured key fails to sign (`:256-286`); a keyless instance stores the review labeled `unsigned_reason = 'no_signing_key'` (`:171-174`) instead of silently computing and discarding a signature. `attestation_packages` gained the columns to hold this at migration 0016 (`src/lib/db/schema.ts:261-300`).

So, to answer the question directly:

- **Whose key signs a correctness endorsement?** The instance key, when one is configured — and the signature now persists: a reader or verifier can check it against the stored envelope. A keyless instance stores the review unsigned and labels it so; an instance where a configured key fails to sign refuses the write rather than storing an unsigned row silently.
- **What identifies the reviewer?** Their identity is inside the hashed, signed payload — database user id, GitHub id, display name, and profile URL (`src/lib/evidence/expert-attestation.ts:86-89`).
- **What does a review change on the record page?** For an expert attestation, nothing about the verification state: only the two machine kinds (`consistency`, `evaluation`) advance a record's verification status, and a human review is deliberately excluded from that state machine (`route.ts:307-314`).

The last of those three is what *disclosure, not validation* requires: a named human vouching for an analysis does not move a platform-rendered verification indicator. What remains open is **[Q73](architecture/open-questions.md#q73--correctness-reviews-are-hashed-and-stored-but-their-signature-and-timestamp-are-discarded)** — not whether the signature persists (it now does), but whether a correctness endorsement should carry the *instance operator's* signature at all, given the disclosure-not-validation posture; the specification's separate migration of pre-v0.1 attestation records to the ratified sub-type URIs (§8.12.2, `:1334`) is where that question would be decided.

**A signed review today is the instance vouching that it recorded the review as submitted — not the reviewer's own countersignature, and not a platform correctness judgment.**
