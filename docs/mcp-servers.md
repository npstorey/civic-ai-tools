...
| [opencontext](https://github.com/CityOfBoston/OpenContext) | Analyze Boston (data.boston.gov) — 311 requests, crime, permits, transit, budget, housing, property assessments (CKAN-based) | http | City of Boston DoIT | Beta | Official. Generalized framework designed for other cities to fork and deploy against their own portals ("one fork = one MCP server"). 6 tools: ckan__search_datasets, ckan__get_dataset, ckan__query_data, ckan__get_schema, ckan__execute_sql, ckan__aggregate_data. Ships SQL/SoQL validators with regex safety checks. Supersedes boston-core-mcp prototype by the same author. Upstream dev hub at github.com/thealphacubicle/OpenContext. MIT-licensed and self-hostable. |
| [nyc-record-mcp](https://github.com/BetaNYC/nyc-record-mcp) | NYC Open Data — City Record Online (dataset dg92-zbpx) | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | No API key required (optional SOCRATA_APP_TOKEN for higher rate limits). Needs network: every tool queries NYC Open Data. README credits socrata-mcp-server as inspiration for its SODA API pattern. Published on npm as @betanyc/nyc-record-mcp since 2026-05-21; latest release 1.1.0 (npm and GitHub, 2026-07-22). |
| [nyc-311-mcp](https://github.com/BetaNYC/nyc-311-mcp) | NYC 311 Public API (NYC API Portal) | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | Requires a free NYC_311_API_KEY subscription key from api-portal.nyc.gov. Needs network. serverInfo reports version 1.0.0; the npm package is 1.1.0. Published on npm as @betanyc/nyc-311-mcp since 2026-06-22; latest release 1.1.0 (npm and GitHub, 2026-07-22). |
...
| [nyc-council-mcp](https://github.com/BetaNYC/nyc-council-mcp) | NYC Council Legistar API, jehiah/nyc_legislation archive (local index) | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | No API key required since 2.0.0. LEGISTAR_TOKEN (free) enables 8 live Legistar tools, LEGISTAR_DB_PATH enables 13 local-index tools, and both together give 20 (get_bill_history is in both sets). The local index is built from a one-time clone of the nyc_legislation archive (~920 MB); local-only mode needs no network after that. Credits @jehiah / intro.nyc as predecessor work on the Legistar API. Published on npm as @betanyc/nyc-council-mcp since 2026-05-19; latest release 2.6.0 (npm and GitHub, 2026-07-27). |
| [nyc-charter-laws-rules](https://github.com/BetaNYC/nyc-charter-laws-rules) | NYC Charter, NYC Administrative Code, Rules of the City of New York | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | No API key required. Runs with no network: the section index ships in the npm package (22,174 sections in 0.2.0, indexed through 2026-07-16). The repository refreshes the index on main from American Legal Publishing (22,185 sections on 2026-09-18); npm 0.2.0 does not include those refreshes. Features a three-layer legal-disclaimer pattern (tool descriptions + response footer + get_version instruction) so responses cannot be returned without grounding caveats. serverInfo reports version 0.1.0; the npm package is 0.2.0. Published on npm as @betanyc/nyc-charter-laws-rules since 2026-05-21; latest release 0.2.0 (npm and GitHub, 2026-07-22). |
| [nys-openlegislation-mcp](https://github.com/BetaNYC/nys-openlegislation-mcp) | NYS Open Legislation API (New York State Senate) | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | Requires a free NYS_LEGISLATION_API_KEY. An optional local SQLite corpus, built with the key from a repository clone (the sync scripts are not in the npm package), enables local-only mode, which needs no network; 6 of the 24 tools have no local coverage. Published on npm as @betanyc/nys-openlegislation-mcp since 2026-05-22; latest release 2.3.0 (npm and GitHub, 2026-07-22). |
...
| [nyc-checkbook-mcp](https://github.com/BetaNYC/nyc-checkbook-mcp) | Checkbook NYC API (NYC Comptroller) | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | No API key required. Needs network: every tool calls the Checkbook NYC API, which the server paces to one request per second. tools/list returns 10 tools; smart_search stays disabled unless CHECKBOOK_ENABLE_SMART_SEARCH=1, and the README documents the other 9. Published on npm as @betanyc/nyc-checkbook-mcp since 2026-05-21; latest release 1.6.1 (npm and GitHub, 2026-08-01). |
| [nyc-budget-mcp](https://github.com/BetaNYC/New-York-City-Budget/tree/main/mcp) | NYC Council adopted-budget documents, as structured in BetaNYC/New-York-City-Budget | stdio | [@BetaNYC](https://github.com/BetaNYC) | Active | The MCP server in the New-York-City-Budget repository (mcp/). No API key required. Runs with no network: a SQLite database of the repository's data ships in the npm package. Schedule C awards cover FY2015–FY2027. The npm package is MIT; the repository README licenses its data CC BY-SA 4.0 (LICENSE-DATA). The repository's main branch declares 1.4.0, which is not on npm, so a clone reports a different serverInfo version from npx. Published on npm as @betanyc/nyc-budget-mcp since 2026-07-08; latest release 1.3.1 (npm, and GitHub tag mcp-v1.3.1, 2026-08-11). |
...
| **Local 311 / permits** | Open311 API, municipal permit databases | NYC only: nyc-311-mcp wraps the NYC 311 Public API (added 2026-09-18). No Open311 or permit server found |
...
- **53 of the 74 servers listed are stdio-only** — they work with Claude Code, Cursor, and VS Code Copilot out of the box, but not with web applications. The other 21 document an HTTP or SSE transport; see the Transport column above for which.
