...
- [`chat-type-taxonomy.md`](chat-type-taxonomy.md) — the conversation-surface discipline. Defines the six project-specific chat types, where each roots, what it reports, and the closure rules that hold them apart. Companion doctrine to this method and to `xanadu-doctrine.md`.
- [`../adr/`](../adr/) — Architectural Decision Records. The ADR log. Current set (as of 2026-05-25 / v0.1-mvp-cohort): ADR-0001 (roadmap governance), ADR-0002 (commitments vs. targets), ADR-0003 (evidence capture method), ADR-0004 (datHere content profile), ADR-0005 (executed-notebook architecture), ADR-0006 (Producer Profile architecture), ADR-0007 (`contentCanonicalization` envelope field), ADR-0008 (multihash `contentHash` + RFC 8785 JCS envelope canonicalization), ADR-0009 (unified typed-attestation primitive), ADR-0010 (visibility / lifecycle / location attestations), ADR-0011 (captureMethod generalization).
...
- [`typed-standards-specification.md`](typed-standards-specification.md) — the consolidated canonical specification. v0.1 Working Draft — open for external review per [ADR-0012](../adr/0012-typed-standards-consolidation.md). Absorbs the former Open Evidence Standard envelope spec and the Civic Claim Vocabulary typed-claims layer.
- [`open-evidence-standard.md`](open-evidence-standard.md) and [`civic-claim-vocabulary-draft-spec.md`](civic-claim-vocabulary-draft-spec.md) — historical snapshots of the pre-consolidation drafts (frozen 2026-05-26 per [ADR-0012](../adr/0012-typed-standards-consolidation.md) §4). Kept for cross-reference accuracy with ADRs 0003-0011, which cite OES + CCV at the section numbers and content state they were written against. Both were internal working drafts (pre-v0.1) at the time this method was formalized.
