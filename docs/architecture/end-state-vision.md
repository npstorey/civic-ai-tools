...
```mermaid
flowchart LR
    P["🎭 Pseudonymous<br/>self-generated keypair<br/>no external binding<br/>(self-certifying did:key form: ADR-0030)"]:::partial
...
**KOI integration.** The *framing* is settled (2026-04→06 working sessions; recorded 2026-07-01): **Typed Standards is the artifact layer — one verifiable claim about one analysis; KOI is the network layer that makes those artifacts collectively legible. Neither layer subordinates the other.** Typed Standards was derived independently of KOI while sharing its foundational commitments (typed nodes, content-addressed identity, consent-aware federation). Five TS contributions are not derivable from KOI: (1) OTel GenAI / MCP semantic-convention traces of agent execution; (2) offline-verifiable self-contained artifacts (the §8.8 bundle, demonstrated per Q15); (3) adversarial evaluation with independent evaluators (bring-your-own-key); (4) Croissant 1.1 PROV-chain linkage; (5) Rekor-anchored immutability. The *implementation* remains speculative: evidence packages as KOI digital objects with RIDs, optionally a koi-net sensor node — the smallest probe (one sensor node watching `civicaitools.org/evidence/*`) is parked in [Q2](open-questions.md#q2--federation-substrate)'s notes, adopter-gated.
