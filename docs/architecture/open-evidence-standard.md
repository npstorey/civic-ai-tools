...
- **`captureMethod`** *(normative)*: The label identifying *how* the package's content was captured — the integrity-of-pipeline property. One of `chat-flow-stream`, `claude-code-jsonl-readback`, `claude-code-self-report`. Specified by [ADR-0003](../adr/0003-evidence-capture-method.md). See §9.
...
10. **Lifecycle state.** Detect withdrawal or reinstatement via the verify endpoint's lifecycle fields and verify the corresponding lifecycle signatures and timestamps. Surfaces MUST render the lifecycle state.
...
- For lifecycle attestations (`withdraws`, `reinstates`, `publishes`), surface the lifecycle state per §10 and the visibility/lifecycle/location ADR (anticipated from [civic-ai-tools#71](https://github.com/npstorey/civic-ai-tools/issues/71)) — that ADR operationalizes the OES §10 withdrawal columns under the `attestation/withdraws/v1` framing and closes `civic-ai-tools-website#58` as obsolete (multi-cycle support is free by construction in the attestation framing).

Operationalization of specific sub-types (the withdrawal-lifecycle implementation, the location-as-attestation implementation, the publication-record flow, the adversarial-eval requirement model) lands in downstream ADRs per the strategic memo §5 sequencing items 4 and following. The ratified Q36 sub-type table per [ADR-0009](../adr/0009-unified-typed-attestation-primitive.md) §7 is the taxonomy backbone; individual sub-types operationalize on their own per-ADR timelines.
