...
- **Capture-method discipline.** Every package declares *how* its content was captured in a signature-covered field — for AI-assisted analyses the vocabulary today names capture from the model's wire stream, readback from the session log on disk, and paraphrased self-report; a sandbox-executed pipeline is a planned extension. Because the label sits inside the signed bytes, it cannot be silently re-described. What the label buys is structural rather than cryptographic: the signature attests that the package was signed and has not been altered, while whether the content matches what was generated in the original session "is structural and follows from the capture method" (§8.6). A verifier reads the label (§9.2 #11) and confirms it is in the profile's vocabulary (§9.2 #15); neither check tests that the labeled mechanism ran. See [`trust-and-evidence.md`](../trust-and-evidence.md) for what each method establishes in the reference implementation. Each Producer Profile declares its own vocabulary of capture methods; future methods extend the per-profile vocabulary, the core discipline holds.
...
- **Truth.** The signature attests that the package was signed and registered by a known publisher and has not been altered since — not that its content is correct. ("Published" there is the act of signing and registering, not the `public` visibility state; visibility is a separate axis per ADR-0016 §A, and a sealed record is signed too.) Editorial review, fact-checking, replication, and adversarial evaluation ride alongside as separately-signed attestations.
