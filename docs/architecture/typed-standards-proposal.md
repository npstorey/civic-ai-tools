...
- `captureMethod` discipline (required, signed, tamper-evident at the canonical-JSON level) per ADR-0003. The field stays in core; per [ADR-0011](../adr/0011-capturemethod-generalization.md), the value space is open at the core level and the vocabulary is declared per Producer Profile in that profile's guidance bundle (per ADR-0006 §5). The v0.1 vocabulary for the `ai-assisted-analysis` Producer Profile — the only built profile today — is `chat-flow-stream`, `claude-code-jsonl-readback`, `claude-code-self-report`, the three values originally enumerated in OES §9 by ADR-0003 and relocated to this profile's guidance bundle by ADR-0011.
...
- A future Sandbox capture mechanism as a high-attestation entry in the `sandbox-only` Producer Profile's captureMethod vocabulary (per [ADR-0011](../adr/0011-capturemethod-generalization.md)'s per-profile vocabulary model; not a core-tier addition), with a two-trust-domain model — the analyst signs the package, the sandbox operator signs an independent execution attestation. Reserved name-only; not yet drafted. Promotion of the `sandbox-only` Producer Profile from reserved to built (per ADR-0006 §1) is the upstream gate; this captureMethod entry lands in that profile's guidance bundle when it does.
- The publisher registry as an indexing-only coordination surface at `typedstandards.org`. Gated on the [Q39](open-questions.md#q39--consolidate-oes--ccv-into-typed-standards-proposalmd-as-a-single-rfc-ready-spec) consolidation trigger (MVP cohort completion + RFC-publish readiness); pre-adoption research checklist (domain registration confirmation, IANA well-known URI check, positioning vs C2PA / in-toto / W3C VC / SLSA / Sigstore) tracked under [Q40](open-questions.md#q40--pre-adoption-research-for-typedstandardsorg-rename--namespace-prefix-selection-ts-vs-tss-vs-alternatives).
- The renamed trust-registry well-known path (`evidence-public-keys.json` → `typed-publisher.json`) and the namespace prefix change (`ccv:` → `ts:` or `tss:` — selection gated on the [Q40](open-questions.md#q40--pre-adoption-research-for-typedstandardsorg-rename--namespace-prefix-selection-ts-vs-tss-vs-alternatives) research checklist's prefix-collision survey, with the load-bearing concern being `tss:`'s collision with **Time-Stamping-Server (TSS)** in RFC 3161 / cryptographic-provenance literature given the spec actively uses RFC 3161; in current canonical-JSON provenance graphs, also the `civic:` prefix that does appear in code today). The coordinated rename + consolidation lands as one ADR + commit cohort per [Q39](open-questions.md#q39--consolidate-oes--ccv-into-typed-standards-proposalmd-as-a-single-rfc-ready-spec).
