...
Where this sketch and the spec disagree, the spec governs.
...
flowchart LR
    ...
    subgraph Study["Study artifacts (exploratory)"]
        ...
        SYSML["SysML v2 model<br/>(structure, behavior,<br/>requirements traceability)"]:::study
    end
    ...
    BEHAV & LIFE & REQ -.formalized by.- SYSML
...
- **Hard boundary** (`03 §3`): crypto math, hash recomputation, TSA cert chains, and
  ...
  subgraph is *outside* the spec-content subgraph, and the study artifacts link only to
  spec content, never to the anchors. A green SHACL report plus a traced SysML model
  still do not constitute verification.
- **Study-artifact placement** (`README`, `03 §5`): SHACL formalizes instance conformance
  of schema; SysML formalizes structure, behavior, lifecycle, and requirements
  traceability. Both are exploratory, Xanadu-gated on named promotion triggers (Q16/Q5
  for SHACL; an MBSE adopter or second verifier for SysML). Rendered dashed accordingly.
