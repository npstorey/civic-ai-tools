...
**Naming — resolved.** The naming thread this section previously held open — whether the user- and resource-facing "evidence" framing (the `publish-evidence` skill, the `/api/evidence` surface, the "evidence page") should migrate toward the more precise typed-node vocabulary — was resolved 2026-08-19 by the Q50/Q66 vocabulary settlement ([ADR-0025](docs/adr/0025-vocabulary-settlement-evidence-excision.md), [civic-ai-tools#160](https://github.com/npstorey/civic-ai-tools/issues/160), spec v0.1.6 Appendix J): the artifact/infrastructure brand role of "evidence" is excised — the artifact is a **record package**, the skill is `publish-record`, the surface `/api/records/` — with every prior-era name kept as a permanent alias, while the epistemic Question/Evidence/Claim role (`content/evidence/v1`) is retained. The hold-until-forced discipline this paragraph used to describe is exactly what played out: the settlement was taken when an external adopter milestone and a concrete freeze risk forced it, not before. (The ADR-0016 migration that landed in Q3 changed *visibility* vocabulary; this settlement changed the artifact framing.)
