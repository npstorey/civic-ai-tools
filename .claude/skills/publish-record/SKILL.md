---
name: publish-record
description: Publish the current Claude Code analysis to the civicaitools.org record registry as a cryptographically signed, timestamped, Rekor-logged record package. Invoke when the user has just completed a civic-data analysis (typically using the Socrata and/or Data Commons MCP tools) and says something like "publish this as a record", "publish this as evidence", "sign this analysis", "publish to civicaitools.org", or "make this a verifiable package."
...
Default capture starts at the **first Socrata or Data Commons MCP tool call** in the session, not at session start. Claude Code sessions often include unrelated setup before the user pivots to civic data; capturing from session start bundles noise into the published record.
...
Sum `promptTokens` + `completionTokens` across **all captured turns**, not just the final turn. The record package's cost attribution reflects the full analysis cost; partial token accounting would misrepresent the dogfooded workload.
...
3. On confirmation, publish for real:
   ...
   ```bash
   python3 .claude/skills/publish-record/publish.py --payload /tmp/publish-record-<timestamp>.json
   ```

   The script resolves auth in this order and uses the first that matches: saved bearer token → `CIVICAITOOLS_SESSION_TOKEN` → `CIVICAITOOLS_SESSION_TOKEN_OP`. Never echo those values. Never pass them on the command line. Oversized fields are uploaded to Vercel Blob before the `/api/records` POST; each upload uses the same credentials as the main POST.

4. On success the script prints a JSON result with `slug`, `recordUrl` (plus `evidenceUrl`, the deprecated alias carrying the identical value), `packageHash`, and `readbackUrl` — plus, for public (not sealed) records, `blobHint`: the canonical, content-addressable package-blob URL. The blob host is not baked into the script; it is read from the target instance's own response (`packageUrl` on the public `GET /api/records/<slug>/commitment`), falling back to a blob URL returned by an upload earlier in the same run. If neither is available the hint is omitted with a warning; `--blob-host` (or `CIVICAITOOLS_BLOB_HOST`) is the escape hatch for instances whose commitment endpoint isn't reachable from where the skill runs. Show the user:
   - The full record URL (`https://civicaitools.org/records/<slug>`)
...
- **Exit 3 (network/HTTP)** — transient or server-side. Applies to both `/api/records` and `/api/blob/upload-token` / Vercel Blob uploads. Show the error to the user; offer to retry once. Do not retry automatically without asking.
...
- The payload JSON contains the prompt text and full output (or transcript) — anything the user would see on the published record page. Do not paste it into another repo or share it outside the immediate conversation.
