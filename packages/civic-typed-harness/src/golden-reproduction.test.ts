 ...
 * `civic:sourceId` is in no registry entry, and asserts how many it found, so
 * a capture the lift no longer matches fails here rather than silently doing
 * nothing. Deleting one key leaves the insertion order of the rest, so the
 * remaining bytes are the capture's own.
 *
 * A near-twin lives in capture/provenance.test.ts; test files run in separate
 * processes, so it is copied rather than imported — the same reason
 * `toSettlementEra` is copied.
 */
...
/** The frozen reference graph, lifted to this case's real prompt hash and over
 *  the #205 fallback. Transforms OF captured bytes — nothing here is
 *  regenerated by the code under test. */
function frozenProvenanceForCase(prompt: string): Record<string, unknown> {
  const frozen = JSON.stringify(
    dropUnregisteredServerUrls(
      WEBSITE_GOLDEN.provenanceGraph as { '@graph': Array<Record<string, unknown>> },
      CIVIC_SOURCE_REGISTRY,
    ),
  );
...
test('#199 §1: the ninth case\'s span-derived expectations are the frozen reference capture, node for node', () => {
  ...
  // lifted only by the two documented transforms — the prompt-hash
  // substitution the fixture documents, and (since 0.5.0,
  // civic-ai-tools#205) the dropped source-id fallback — and the blast radius
  // of both is measured here, so "lifted" cannot quietly come to mean
  // "regenerated".
  //
  ...
  // 1. The lift moved exactly four of the capture's 28 nodes: the prompt
  //    entity the synthetic hash names, the two inferences that use it, and
  //    the ONE agent for a source the registry does not carry, which the
  //    reference implementation gave `civic:serverUrl` equal to its source id
  //    and 0.5.0 omits. Every other span-derived node is frozen capture,
  //    untouched — including the three agents whose address the registry has.
  ...
  assert.deepEqual(
    ...
    [
      ...
      // … the one agent whose source is in no registry entry, which loses the
      // `civic:serverUrl` the source-id fallback used to assert (#205). The
      // other three agents are untouched: the registry carries their
      // addresses, so 0.5.0 emits exactly what the capture recorded.
      'urn:civic-evidence:mcp-server:euro%20stat',
      // … and the two inferences that reference the prompt in `prov:used`.
      // Nothing else in the capture mentions the prompt hash, and in
      // particular no node derived from a span does.
      'urn:civic-evidence:pkg-golden-001:inference:inf-1',
      'urn:civic-evidence:pkg-golden-001:inference:inf-2',
    ],
    'a lift reached a node it has no business reaching',
  );
